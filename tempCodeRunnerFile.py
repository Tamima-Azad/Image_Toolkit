...
class ImageToolkit:
    def __init__(self, root):
        ...
        edge_menu.add_command(label="Canny (requires OpenCV)", command=lambda: self.edge_detection("canny"))
        ...
        try:
            photo_name = "IMG_20240412_165528.jpg"
            possible_paths = [
    ...
    def update_image(self, new_img, name="Edited"):
    ...
    def intensity_transform(self, choice):
        ...
        gamma = None
        if choice == "1":  # Log Transform
            dialog = OperationParameterDialog(
                self.root, "Log Transform", "N/A", "N/A", 0, 0, show_parameters=False
            )
            transform_name = "Log Transformed"
        elif choice == "2":  # Gamma Transform
            dialog = OperationParameterDialog(
                self.root, "Gamma Transform", "Gamma Value", "N/A", 1.0, 0, show_parameters=True
            )
            transform_name = "Gamma Transformed"
        else:
            return
        ...
        arr = self._get_image_for_operation(source_choice)
        if arr is None: return

        # Manual loop-based implementation for log and gamma transform
        if choice == "1":  # Log Transform
            if arr.ndim == 3 and arr.shape[2] == 3:
                ...
                for ch in range(3):
                    for i in range(height):
                        for j in range(width):
                            result[i, j, ch] = c * np.log(1 + arr[i, j, ch])
                ...
                for i in range(height):
                    for j in range(width):
                        result[i, j] = c * np.log(1 + arr[i, j])
                result = np.clip(result, 0, 255).astype(np.uint8)
                mode = "L"
            self.proc_name = "Log Transformed"

        elif choice == "2":  # Gamma Transform
            try:
                gamma = float(param1)
            except Exception:
                messagebox.showerror("Error", "Gamma must be a numeric value (e.g. 0.5, 1.0, 2.2).")
                return
            if arr.ndim == 3 and arr.shape[2] == 3:
                height, width, channels = arr.shape
                ...
                for ch in range(3):
                    for i in range(height):
                        for j in range(width):
                            result[i, j, ch] = (arr[i, j, ch] ** gamma) * 255
                result = np.clip(result, 0, 255).astype(np.uint8)
                mode = "RGB"
            else:
                height, width = arr.shape
                ...
                for i in range(height):
                    for j in range(width):
                        result[i, j] = (arr[i, j] ** gamma) * 255
                result = np.clip(result, 0, 255).astype(np.uint8)
                mode = "L"
            self.proc_name = f"Gamma={gamma}"

        self.update_image(Image.fromarray(result, mode=mode))
    ...
    def edge_detection(self, method):
        ...
        if method in ["sobel", "prewitt", "roberts"]:
            ...
            arr = self._get_array_for_operation(source_choice)
            if arr is None: return
            messagebox.showwarning("Canny Not Implemented", "Canny Edge Detection requires OpenCV (cv2) which is not imported/included in this pure NumPy/PIL implementation.")
            return
        else:
